...
- **CI/CD Pipeline**: Automated CI/CD is managed via GitHub Actions. Pull Requests trigger CI (tests), merges to `main` deploy to Dev, and Production deployments require manual approval. Use `./scripts/setup-cicd.sh <env> <location>` to configure pipeline resources.
- **Deployment Best Practice**: Deployments should primarily happen via GitHub Actions for consistent environments. Manual `azd up` is for local dev/troubleshooting only.
- **Environments**: Both `dev` and `prod` environments are defined in GitHub for safe, staged deployments.
